            ...
            for batch in progress_bar:
                ...
                with autocast(device_type="cuda"):
                    ...
                    if self.ema_kappa is None:
                        ...
                        self.ema_kappa = (
                            self.ema_decay * self.ema_kappa + (1 - self.ema_decay) * k
                        )
                        self.ema_proto = (
                            self.ema_decay * self.ema_proto + (1 - self.ema_decay) * p
                        )

                    raw_lambda = (
                        self.config.REGULAR_RATIO
                        * self.ema_kappa
                        / (self.ema_proto + self.eps)
                    )

                    loss_weighted_proto = raw_lambda * loss_proto
